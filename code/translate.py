...
def process_single_row(data_row, prompt_text, api_list_src, model_list, base_url, thread_id):
    """
    处理单行数据的函数
    """
    ...
    try:
        result_json = get_struct_json(
            user_prompt=prompt_text + dict(data_row)['sourceText'],
...
def process_data_multithread(df_data, prompt_text, api_list_src, model_list, base_url, max_workers=5):
    """
    多线程处理数据
    """
    dst_data = []
...
def process_single_row_with_multiple_models(data_row, prompt_text, dict_config_list, thread_id):
    """
    处理单行数据并使用多个模型配置的函数
    """
    print(f"线程 {thread_id} 正在处理第 {data_row.name} 行数据")
...
if __name__ == '__main__':
    # 读取API配置
    df_api = pd.read_excel(r"api_list/config_1api_v2.xlsx")
    dict_config_list = []
    model_list = []
    ...
    # change the source data and run
    prompt_text = read_docx_text(r"prompt/针灸翻译提示词 - 无rag.docx")
    df_data = pd.read_excel(r"src_data/前300.xlsx")
    dst = process_data_with_multiple_models_multithread(
        df_data=df_data,
    ...
    df = pd.DataFrame(dst)
    df.to_excel(r"dst_data/无RAG.xlsx", index=False)
