...
def generate_sql(question, api_list, model, base_url):

    sql_prompt_file = r"prompt/指南SQL生成例子-a.docx"
    prompt_text = read_docx_text(sql_prompt_file)

    content = prompt_text.replace("{content}", question)
    ...
    try:
        ...
        sql_statement = list1[-1].split("```")[0].strip().replace("\n", "")
...
if __name__ == '__main__':
    ...
    # prompt_text = ""
    prompt_text = read_docx_text(r"prompt/指南回答提示词-z-rag.docx")
    ...
    for df_idx, df_row in df_data.iterrows():
        ...
        sql_statement = generate_sql(question=cur_dict['question'],
                                    api_list=dict_config_list[0]['api_list_src'],
                                    model='google/gemini-2.5-flash-lite',
                                    base_url=dict_config_list[0]['base_url'])

        search_result = query_knee_osteoarthritis(sql_statement)
        ...
        for m_c in dict_config_list:
            ...
            if cur_dict["hit"]=="yes":
                user_prompt = prompt_text.replace("{content}", str(dict(df_row)['question']))
                user_prompt = user_prompt.replace("{rag_content}", search_result_str)
                print(user_prompt)
            else:
                user_prompt = prompt_text.replace("{content}", str(dict(df_row)['question']))
                user_prompt = user_prompt.replace("{rag_content}", "无")
            result = get_llm_result(user_prompt=user_prompt,
            ...
            if "\nGuideline title:" in result:
                list_1 = result.split("Guideline title:")
                result = f"Guideline title:{list_1[-1]}"


            if "\n指南名称：" in result:
                list_1 = result.split("指南名称：")
                result = f"指南名称：{list_1[-1]}"
            cur_dict[f"{model}_answer"] = result
            print(f"{model}:{dict(df_row)['question']}")
            print(result)
            input("")
        dst.append(cur_dict)
    df = pd.DataFrame(dst)
    df.to_excel(r"dst_data/text2sql-rag-z.xlsx", index=False)
